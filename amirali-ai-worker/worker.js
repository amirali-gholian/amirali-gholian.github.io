...
const KB_VERSION = "2026-09-05-v3";

// Google Gemini Free Tier shown in the user's project dashboard: 20 requests/day.
// On the current day, the dashboard showed 14/20, so the first deployment reserves
// only the observed 6 remaining requests. From the next Pacific calendar day onward,
// the worker allows the full 20/day budget.
const DAILY_LIMIT = 20;
const CURRENT_BOOTSTRAP_DATE = "2026-09-05";
const CURRENT_BOOTSTRAP_REMAINING = 6;
...
async function quotaFetch(env, method, body) {
  const id = env.AI_DAILY_QUOTA.idFromName("global");
  const stub = env.AI_DAILY_QUOTA.get(id);

  return stub.fetch(`https://quota.internal/${method === "GET" ? "status" : "consume"}`, {
    method,
...
export class AIDailyQuota {
  ...
  async fetch(request) {
    ...
    let count = Number(stored?.count || 0);
    let limit = DAILY_LIMIT;

    if (today === CURRENT_BOOTSTRAP_DATE) {
      limit = CURRENT_BOOTSTRAP_REMAINING;
    }
    ...
    await this.state.storage.put("quota", {
      date,
      count
    });
...
export default {
  async fetch(request, env) {
    ...
    // Per-IP burst protection. This protects the Gemini Free Tier from one visitor
    // repeatedly hammering the endpoint.
    if (env.AI_IP_LIMITER) {
      const ip = request.headers.get("CF-Connecting-IP") || "unknown";
      const ipLimit = await env.AI_IP_LIMITER.limit({ key: ip });

      if (!ipLimit.success) {
        return json(
          {
            error: "Too many requests from this connection. Please wait a minute and try again.",
            code: "IP_RATE_LIMIT"
          },
          429,
          origin
    ...
    try {
      ...
      const quotaStatusResponse = await quotaFetch(env, "GET");
      const quotaStatus = await quotaStatusResponse.json();
      ...
      // Reserve one Gemini request from the global daily budget only when
      // the request will actually call Gemini.
      const quotaResponse = await quotaFetch(env, "POST", { action: "consume" });
