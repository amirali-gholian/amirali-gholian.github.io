...
const KB_VERSION = "2026-09-05-groq-v3";
...
export default {
  async fetch(request, env) {
    ...
    if (!env.GROQ_API_KEY) {
      return json({ error: "GROQ_API_KEY is not configured." }, 500, origin);
    }
    ...
    try {
      ...
      const languageInstruction = language === "fa"\n        ? "Answer naturally in Persian."\n        : "Answer naturally in English.";\n\n      const groqMessages = [\n        {\n          role: "system",\n          content: `${SYSTEM_PROMPT}\\n\\n${languageInstruction}`\n        },\n        ...rawMessages.slice(-12).map(m => ({\n          role: m.role === "assistant" ? "assistant" : "user",\n          content: String(m.content || "").trim().slice(0, 5000)\n        })).filter(m => m.content)\n      ];\n\n      const response = await fetch(GROQ_URL, {\n        method: "POST",\n        headers: {\n          "Content-Type": "application/json",\n          "Authorization": `Bearer ${env.GROQ_API_KEY}`\n        },\n        body: JSON.stringify({\n          model: MODEL,\n          messages: groqMessages,\n          temperature: 0.4,\n          max_tokens: 4096,\n          reasoning_effort: "medium"\n        })\n      });\n\n      const data = await response.json();\n\n      if (!response.ok) {\n        console.error("GROQ_API_ERROR", response.status, JSON.stringify(data));\n        if (response.status === 429) {\n          return json({ error: "Groq is temporarily rate-limited. Please try again shortly." }, 429, origin);\n        }\n        return json({ error: "Groq API request failed." }, 502, origin);\n      }\n\n      const answer = data?.choices?.[0]?.message?.content?.trim() || "";\n\n      if (!answer) {\n        console.error("GROQ_EMPTY_RESPONSE", JSON.stringify(data));\n        return json({ error: "Groq returned an empty response. Please try again." }, 503, origin);\n      }\n\n      if (singleTurn) {\n        await putCached(await sha256(cacheKey), answer);\n      }\n\n      return json(
          {
